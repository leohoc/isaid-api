Feature: Testing the Prophecy API
  ...
  Scenario Outline: Creating Prophecies
    When clients makes a POST request to "<prophetLogin>" prophecies with "<summary>" as summary and "<description>" as description
    ...
    Examples:
      ...
      |              |                                      | I bet misconfigured bots will try this        | There'll be all sort of strange behavior | 400              | false        |
  ...
  Scenario Outline: Retrieving Prophecies Errors
    When clients makes a GET request to "<login>" prophecies
    Then a <httpResponseCode> http response will be returned by the Prophecy resource
